...
def draw_overview_by_view(
    ...
    """Draw a semantic full-model overview grouped by Vensim views.

    Styling rules:
    - Macro_Ebene is shown as the largest, most prominent view panel.
    - Views 01-04 are highlighted as AMM dynamising components.
    - AMM core logic views are highlighted with a separate color and node shape.
    - All dependency lines are directed from source/input variable to target/equation variable.
    """
    ...
    detail_views = [v for v in view_names if v != "00_Legend"]
    if not detail_views:
        detail_views = sorted({variable.primary_view for variable in variables.values()})
    ...
    macro_h = 0.86 if macro_present else 0.0
    grid_rows = math.ceil(len(non_macro_views) / cols) if non_macro_views else 0
    extra_h = 0.55 if any(v.primary_view == "Unassigned" for v in variables.values()) else 0.0
    fig_w = 8.5 * cols
    fig_h = 5.2 * grid_rows + (3.8 if macro_present else 0.0) + (2.2 if extra_h else 0.0)
    fig, ax = plt.subplots(figsize=(fig_w, max(fig_h, 8)))
    ax.set_axis_off()
    ...
    def add_view_panel(view: str, x0: float, y0: float, w: float, h: float) -> None:
        ...
        rect = plt.Rectangle(
            ...
            linewidth=style["panel_lw"],
            alpha=0.88,
            zorder=0,
        )
        ax.add_patch(rect)
        title_size = 15 if view == MACRO_VIEW else 11.5
        ax.text(
            x0 + 0.04,
            y0 - 0.035,
            view,
        ...
        for occ in view_occs:
            px = x0 + 0.08 * w + 0.84 * w * ((occ.x - min_x) / span_x)
            py = y0 - 0.17 * h - 0.70 * h * ((occ.y - min_y) / span_y)
            node_pos[occ.name] = (px, py)

    # Macro_Ebene gets a wide top panel instead of being squeezed into the grid.
    y_cursor = 0.0
    if macro_present:
        add_view_panel(MACRO_VIEW, 0.0, y_cursor, cols * cell_w, macro_h)
        y_cursor -= macro_h
    ...
    # Place unassigned variables in a small extra area.
    unassigned = [name for name, v in variables.items() if name not in node_pos]
    if unassigned:
        y0 = y_cursor - grid_rows * cell_h
        add_view_panel("Unassigned", 0.0, y0, cols * cell_w, extra_h)
        for i, name in enumerate(sorted(unassigned)):
            node_pos[name] = (0.12 + (i % 26) * 0.105, y0 - 0.16 - (i // 26) * 0.085)

    # Prepare semantic categories and node sizes before drawing arrows.
    # The arrow shrink values below are based on these sizes, so arrowheads stop at
    # the visual border of the target symbol instead of ending inside the symbol.
    ...
    def node_size_for_name(name: str) -> float:
        ...
        degree = variable.in_degree + variable.out_degree
        base = 35 if category != "macro" else 130
        scale = 130 if category != "macro" else 230
    ...
    def arrow_margin_points(name: str, factor: float, minimum: float) -> float:
        # Matplotlib scatter sizes are in points^2. sqrt(size) is therefore a
        # useful proxy for the visible marker diameter in points. Diamonds and
        # larger semantic markers need a little more margin than circles.
        variable = variables[name]
        category = variable_category(variable)
        marker_bonus = 5.0 if category in {"macro", "amm_logic", "technical"} else 2.5
    ...
    for source, target in edges:
        ...
        alpha = 0.18 if same_view else 0.34
        lw = 0.55 if same_view else 0.95
        ...
        ax.annotate(
            ...
            arrowprops=dict(
                ...
                alpha=alpha,
                color=color,
                shrinkA=arrow_margin_points(source, factor=0.50, minimum=8),
                shrinkB=arrow_margin_points(target, factor=0.86, minimum=14),
                mutation_scale=8 if same_view else 10,
                connectionstyle="arc3,rad=0.06" if not same_view else "arc3,rad=0.02",
            ),
            zorder=1,
        )

    # Draw nodes by semantic role so AMM logic and AMM dynamics are visually separable.
    ...
    # Label stocks, flows, lookups, KPIs, macro variables and high-degree hubs.
    ...
    for name, (x, y) in node_pos.items():
        variable = variables[name]
        if (
            variable.primary_view == MACRO_VIEW
            ...
            label_size = 8.6 if variable.primary_view == MACRO_VIEW else 7.3
            ax.text(x + 0.008, y + 0.008, _shorten_label(name), fontsize=label_size, zorder=4)

    # Legend for semantic view/node styling and directed edges.
    legend_handles = [
        Line2D([0], [0], marker=semantic_styles["macro"]["marker"], color="w",
               label=semantic_styles["macro"]["label"], markerfacecolor=semantic_styles["macro"]["node"],
               markeredgecolor="white", markersize=13),
        Line2D([0], [0], marker=semantic_styles["amm_dynamic"]["marker"], color="w",
               label=semantic_styles["amm_dynamic"]["label"], markerfacecolor=semantic_styles["amm_dynamic"]["node"],
               markeredgecolor="white", markersize=10),
        Line2D([0], [0], marker=semantic_styles["amm_logic"]["marker"], color="w",
               label=semantic_styles["amm_logic"]["label"], markerfacecolor=semantic_styles["amm_logic"]["node"],
               markeredgecolor="white", markersize=9),
        Patch(facecolor="#FFFFFF", edgecolor="#2D3748", label="Pfeile: Input-Variable → berechnete Zielvariable"),
    ]
    ax.legend(
    ...
    ax.set_title("Gesamtübersicht des Vensim-Modells nach Views", fontsize=18, fontweight="bold", pad=14)
    total_rows_h = macro_h + grid_rows * cell_h + extra_h
    ax.set_xlim(-0.02, cols * cell_w + 0.02)
    ax.set_ylim(-total_rows_h - 0.12, 0.12)
    plt.tight_layout()
