...
def draw_overview_by_view(
    ...
    IMPORTANT_STOCKS = {"Urban Land", "Agricultural Land", "Aggricultural Land"}

    def is_important_stock_name(name: str) -> bool:
    ...
    hub_rank_count = 30
    hub_names = {v.name for v in ranked[:hub_rank_count]}
    hub_degree_cutoff = min((v.in_degree + v.out_degree for v in ranked[:hub_rank_count]), default=0)
    ...
    def should_label(variable: VensimVariable) -> bool:
        return (
            is_important_stock_name(variable.name)
    ...
    def node_size_for_name(name: str) -> float:
        ...
        degree = variable.in_degree + variable.out_degree
        if is_important_stock_name(name):
            return 650
    ...
    def add_view_panel(view: str, x0: float, y0: float, w: float, h: float) -> None:
        ...
        # View titles are larger and placed clearly inside the panel, below the border.
        title_size = (10.2 if view == MACRO_VIEW else 8.2) * FONT_SCALE
        title_y = y0 - (0.070 if view == MACRO_VIEW else 0.060)
        ax.text(
            x0 + 0.055,
            title_y,
            view,
            ...
            va="top",
            zorder=5,
            path_effects=[pe.withStroke(linewidth=4.2, foreground=style["face"], alpha=0.95)],
        )
        if view == MACRO_VIEW:
            ax.text(
                x0 + w - 0.05,
        ...
        for occ in view_occs:
            ...
            py = min(max(py, y0 - 0.88 * h), y0 - 0.24 * h)
    ...
    def clamp_to_panel(view: str, x: float, y: float) -> tuple[float, float]:
        ...
        y = min(max(y, y0 - 0.865 * h), y0 - 0.245 * h)
        return x, y
    ...
    def arrow_margin_points(name: str, factor: float, minimum: float) -> float:
        variable = variables[name]
        category = variable_category(variable)
        marker_bonus = 8.0 if is_important_stock_name(name) else (5.0 if category in {"macro", "amm_logic", "technical"} else 2.5)
    ...
    for view, items in labels_by_view.items():
        ...
        for item in sorted(items, key=lambda d: (-d["y"], d["x"])):
            ...
            y = min(max(y, y0 - 0.850 * h), y0 - 0.235 * h)
            item["y"] = y
            placed.append(item)
        ...
        for idx in range(1, len(placed)):
            ...
            if same_x_area and abs(item["y"] - prev["y"]) < min_gap:
                item["y"] = min(prev["y"] + min_gap, y0 - 0.235 * h)
    ...
    # It explains the visual reading rules and why only selected nodes are labelled.
    # -------------------------------------------------------------------------
    legend_x = 1.04
    ...
    title_fs = 7.0 * FONT_SCALE
    text_fs = 4.9 * FONT_SCALE
    item_fs = 4.7 * FONT_SCALE
    tx = legend_x + 0.045
    ty = legend_y - 0.050
    ax.text(tx, ty, "Legende und Leseregel", fontsize=title_fs, fontweight="bold", zorder=7, va="top")
    ty -= 0.095

    # Box colors as small colored panels instead of prose.
    box_items = [
        ("Macro", semantic_styles["macro"]["face"], semantic_styles["macro"]["edge"]),
        ("AMM-Dynamisierung", semantic_styles["amm_dynamic"]["face"], semantic_styles["amm_dynamic"]["edge"]),
        ("AMM-Kernlogik", semantic_styles["amm_logic"]["face"], semantic_styles["amm_logic"]["edge"]),
        ("Cobb-Douglas", semantic_styles["cobb_doug"]["face"], semantic_styles["cobb_doug"]["edge"]),
        ("Innovation", semantic_styles["westcourt"]["face"], semantic_styles["westcourt"]["edge"]),
    ]
    x = tx
    for label, face, edge in box_items:
        sw = 0.030
        ax.add_patch(
            plt.Rectangle(
                (x, ty - 0.022),
        ...
        ax.text(x + sw + 0.010, ty - 0.007, label, fontsize=item_fs, fontweight="bold", va="center", zorder=7)
        x += 0.34 if label != "AMM-Dynamisierung" else 0.44

    # Node symbol examples are placed directly below the box-color row.
    # This prevents the symbols from colliding with the explanatory text below.
    ty -= 0.070
    examples = [
        ("zentrale Stocks", "#D7191C", "o", "black"),
        ("AMM-Kernlogik", "#ED8936", "D", "white"),
        ("AMM-Dynamisierung", "#4299E1", "o", "white"),
    ]
    ex_x = tx + 0.020
    for label, color, marker, edge in examples:
        ax.scatter([ex_x], [ty], s=210, c=color, marker=marker, edgecolors=edge, linewidths=1.4, zorder=7)
        ax.text(ex_x + 0.045, ty, label, fontsize=item_fs, fontweight="bold", va="center", zorder=7)
        ex_x += 0.36
    ...
    label_rule_lines = [
        "• Stocks und Flows",
        "• wichtige Outputs",
        f"• zentrale Hubs mit vielen Kanten, Cutoff: Top {hub_rank_count} nach Edge-Anzahl, mindestens {hub_degree_cutoff} Edges",
        "• AMM-Kernvariablen",
    ]
    for line in label_rule_lines:
        ax.text(tx + 0.010, ty, line, fontsize=item_fs, fontweight="bold", zorder=7, va="top")
        ty -= 0.060
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Export an A0-style SVG/PNG overview from a Vensim .mdl file using the original Vensim sketch coordinates.")
    parser.add_argument("mdl_path", type=Path, help="Path to the Vensim .mdl file")
    parser.add_argument("--out-dir", type=Path, default=Path("assets/vensim_a0_export"), help="Output directory")
