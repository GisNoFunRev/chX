...
def draw_overview_by_view(
    ...
    for label, face, edge in box_items:
        sw = 0.030
        ax.add_patch(plt.Rectangle((x, ty - 0.022), sw, 0.030, facecolor=face, edgecolor=edge, linewidth=1.4, zorder=7))
    ...
    # Visual examples, each on its own line.
    ex_x = tx + 0.020
    ex_y = legend_y - legend_h + 0.145
    example_gap = 0.064
    examples = [
    ...
    for idx, (label, color, marker, edge) in enumerate(examples):
        yy = ex_y - idx * example_gap
        ax.scatter([ex_x], [yy], s=210, c=color, marker=marker, edgecolors=edge, linewidths=1.4, zorder=7)
        ax.text(ex_x + 0.045, yy, label, fontsize=item_fs, fontweight="bold", va="center", zorder=7)
